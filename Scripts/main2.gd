...
func create_buttons():
	...
	cancel_button.text = "✕"
	cancel_button.anchor_left = 0
	cancel_button.anchor_top = 0
	cancel_button.anchor_right = 0
	cancel_button.anchor_bottom = 0

	cancel_button.size = Vector2(100, 100)
	cancel_button.position = Vector2(screen_size.x - cancel_button.size.x - 40, screen_size.y - cancel_button.size.y - 130)
	cancel_button.add_theme_font_size_override("font_size", 40)
	cancel_button.add_theme_color_override("font_color", Color(1, 1, 1))

	var cancel_style := StyleBoxFlat.new()
	cancel_style.bg_color = Color(0.9, 0.2, 0.2)
	cancel_style.border_color = Color(0.5, 0, 0)
	cancel_style.border_width_left = 3
	cancel_style.border_width_top = 3
	cancel_style.border_width_right = 3
	cancel_style.border_width_bottom = 3
	cancel_style.corner_radius_top_left = 50
	cancel_style.corner_radius_top_right = 50
	cancel_style.corner_radius_bottom_left = 50
	cancel_style.corner_radius_bottom_right = 50
	cancel_style.shadow_size = 6
	cancel_style.shadow_color = Color(0, 0, 0, 0.3)

	var cancel_hover := cancel_style.duplicate()
	cancel_hover.bg_color = Color(1, 0.3, 0.3)

	var cancel_pressed := cancel_style.duplicate()
	cancel_pressed.bg_color = Color(0.7, 0, 0)
...
func update_score_display():
	...
	score_tween.tween_property(self, "displayed_score", score / 10, 0.5).set_trans(Tween.TRANS_QUAD).set_ease(Tween.EASE_OUT)
...
func show_bonus_popup(kind: String, value: int):
	...
	popup.position = Vector2(screen.x / 2, base_y + offset + randf_range(-5, 5))
	popup.position.x += randf_range(-80, 40)
...
func check_word():
	...
	var has_bomb := false
	var has_wild := false
	var popup_final_mult := 1   # used to show the right multiplier popup later
	...
	if valid:
		var points := calculate_word_score(word)
		for ch in word:
			if ch in ["Q", "Z", "X", "J"]:
				points += 10

		points = int(points * word_mult)
		points += base_points
		points = int(points * (1 + combo * 0.2))
		...
		# default multiplier (for non-bomb words)
		popup_final_mult = int(word_mult)
		...
		if has_bomb:
			...
			var word_base := calculate_word_score(word) + base_points
			for ch in word:
				if ch in ["Q","Z","X","J"]:
					word_base += 10

			var explosion_points := _sum_explosion_points(explosion_dict)
			var overall_mult := _overall_multiplier(selected_letters, explosion_dict)

			var total_points := int((word_base + explosion_points) * overall_mult)
			total_points = int(total_points * (1 + combo * 0.2))
			score += total_points
			time_left = min(60.0, time_left + word.length())
			combo += 1
			update_score_display()
			...
			popup_final_mult = int(overall_mult)  # 🔥 use combined multiplier from explosion

			_remove_tiles_dict(all_to_clear)
		else:
			remove_selected()

		# --- 🟢 SHOW POPUPS (only once per word) ---
		if word.length() > 4:
			show_bonus_popup("length", word.length())
		if combo > 1:
			show_bonus_popup("combo", combo)
		if popup_final_mult > 1:
			show_bonus_popup("multiplier", popup_final_mult)
...
func game_over():
	...
	$GameOverUI/ColorRect/VBoxContainer/FinalScoreLabel.text = "Score: %d" % (score / 10)
